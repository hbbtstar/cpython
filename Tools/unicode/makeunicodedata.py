...
def makeunicodedata(unicode, trace):
    ...
    print >>fp, "const _PyUnicode_DatabaseRecord _PyUnicode_Database_Records[] = {"
    for item in table:
...
def makeunicodetype(unicode, trace):
    ...
    print >>fp, "/* extended case mappings */"
    print >>fp
    print >>fp, "const Py_UCS4 _PyUnicode_ExtendedCase[] = {"
    ...
    print >>fp, ' */'
    print >>fp, 'double _PyUnicode_ToNumeric(Py_UCS4 ch)'
    print >>fp, '{'
    print >>fp, '    switch (ch) {'
    ...
    print >>fp, " */"
    print >>fp, 'int _PyUnicode_IsWhitespace(const Py_UCS4 ch)'
    print >>fp, '{'
    print >>fp, '    switch (ch) {'
    ...
    print >>fp, " */"
    print >>fp, 'int _PyUnicode_IsLinebreak(const Py_UCS4 ch)'
    print >>fp, '{'
    print >>fp, '    switch (ch) {'
...
def makeunicodename(unicode, trace):
    ...
    print >>fp
    print >>fp, 'static const unsigned int aliases_start = %#x;' %
          NAME_ALIASES_START
    print >>fp, 'static const unsigned int aliases_end = %#x;' %
          (NAME_ALIASES_START + len(unicode.aliases))

    print >>fp, 'static const unsigned int name_aliases[] = {'
    for name, codepoint in unicode.aliases:
        print >>fp, '    0x%04X,' % codepoint
    print >>fp, '};'
    ...
    print >>fp, 'static const unsigned int named_sequences_start = %#x;' %
          NAMED_SEQUENCES_START
    print >>fp, 'static const unsigned int named_sequences_end = %#x;' %
          (NAMED_SEQUENCES_START + len(unicode.named_sequences))

    print >>fp, 'static const named_sequence named_sequences[] = {'
    for name, sequence in unicode.named_sequences:
        seq_str = ', '.join('0x%04X' % cp for cp in sequence)
        print >>fp, '    {%d, {%s}},' % (len(sequence), seq_str)
    print >>fp, '};'
